...
작성 종이는 데스크톱에서 본문과 230px 첨부 영역의 2열이며, 960px에서 첨부 영역이 210px로 줄고, 680px에서 한 열로 합쳐진다. 필터는 넓은 화면에서 검색과 선택 도구를 양쪽에 두고, 960px 아래에서 한 열, 680px 아래에서 두 개의 select와 한 줄 전체 정리 토글로 재배치한다. 상세 편집은 오른쪽 최대 470px 패널이며 680px 이하에서 화면 전체 너비를 쓴다.
...
우측에서 들어오는 단일 아이보리 시트다. 38% 잉크색 backdrop으로 목록과 분리하고, 헤더는 상단에 고정한다. 키보드 포커스는 패널 안에 머물고 Escape 또는 닫기 버튼으로 원래 위치에 돌아간다. 모바일에서는 하드 그림자를 제거한 전체 너비 편집면이 된다.
