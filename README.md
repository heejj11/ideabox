...
- `localhost` 또는 `127.0.0.1` 링크가 있는 아이디어는 상세 패널에서 포트 상태를 확인할 수 있습니다. 브라우저
  보안상 프로세스를 직접 종료하지 않고 `lsof -tiTCP:<포트> -sTCP:LISTEN | xargs kill` 명령을 복사합니다.
